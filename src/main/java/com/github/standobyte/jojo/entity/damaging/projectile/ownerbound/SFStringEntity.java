...
public class SFStringEntity extends OwnerBoundProjectileEntity {
    ...
    public SFStringEntity(World world, LivingEntity entity, float angleXZ, float angleYZ, boolean isBinding) {
        super(ModEntityTypes.SF_STRING.get(), entity, world);
	...
	public int ticksLifespan() {
        return isBinding ? ModActions.STONE_FREE_STRING_BIND.get().getStandActionTicks(null, null) 
                + (isAttachedToAnEntity() ? 10 : 0)
                : ModActions.STONE_FREE_STRING_ATTACK.get().getStandActionTicks(null, null);
    }
